...
{
	public class PulseAppCacheStats
	{
	...
	public class PulseAppCache
	{
		...
		public PulseAppCache()
		{
			string cacheRoot = FileSystem.CacheDirectory;
			string databasePath = Path.Combine(cacheRoot, "pulse.db");
		...
		public PulseAppCacheStats GetCacheStats()
		{
			PulseAppCacheStats stats = new PulseAppCacheStats();
			lock (m_sqlLock)
			{
				using (SqliteCommand cmd = m_connection.CreateCommand())
				{
