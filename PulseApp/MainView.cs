...
#if ANDROID
using PulseApp.Playback.AndroidOS;
#endif
...
namespace PulseApp
{
	...
	public class MainView : ContentPage, IMediaClientHost
	{
		...
		public MainView()
		{
			...
			m_cache = new PulseAppCache();
			m_mediaClient = new PulseClient(m_cache, this);
			m_analytics = new Analytics(m_mediaClient);

			m_mediaClient.SetServerParams(PulseAppSettings.GetServerIp(), PulseAppSettings.GetServerPort(), PulseAppSettings.GetUsername(), PulseAppSettings.GetPassword(), PulseAppSettings.GetUseHttps());
#if ANDROID
			m_player = new PulseAppAndroidPlayer(this, m_mediaClient);
#else
			m_player = new StubPulseAppPlayer();
#endif

			m_shuffleEnabled = PulseAppSettings.GetShuffleEnabled();
			m_repeatMode = PulseAppSettings.GetRepeatMode();
		...
		public void Logout()
		{
			...
			PulseAppSettings.SetPassword("");
			PulseAppCache cache = GetCache();
			cache.ExecuteAsync(() =>
			{
				cache.ClearCache();
			});
		...
		{
			if (string.IsNullOrEmpty(PulseAppSettings.GetUserID()))
			{
				NavigateToLogin();
				return false;
			}
		...
		// forwards the signal down to its children (see PulseAppView).
		private void SetActiveContent(View view)
		{
			m_contentHost.Content = view;
			PulseAppView pulseAppView = view as PulseAppView;
			if (pulseAppView != null)
			{
				pulseAppView.OnNavigatedTo();
			}
		}
		...
		private void SetShuffleState(bool enabled)
		{
			...
			PulseAppSettings.SetShuffleEnabled(enabled);
		...
		{
			m_repeatMode = mode;
			PulseAppSettings.SetRepeatMode(mode);
