...
{
	public class PulseAppView : ContentView
	{
		protected MainView m_mainView;

		public PulseAppView(MainView mainView)
		{
			m_mainView = mainView;
            BuildLayout();
        }
		...
		// the signal down). The base implementation forwards to child PulseAppViews;
		...
		// nearest descendant PulseAppViews. Each of those forwards to its own
		...
		protected void ForwardNavigatedToChildren(IView element)
		{
			...
			{
				PulseAppView childView = children[index] as PulseAppView;
				if (childView != null)
				{
