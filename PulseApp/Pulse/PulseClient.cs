...
namespace PulseApp.Pulse
{
	...
	public class PulseClient : MediaClient
	{
		...
		// (L2, via PulseAppCache). Keyed by the resolved cover-art URL so repeated
		...
		PulseAppCache m_cache;

		public PulseClient(PulseAppCache cache, IMediaClientHost host) : base(host)
		{
			m_cache = cache;
		}
		...
		private string BuildPulseUrl(string endpoint, string extraParams, string overrideBaseUrl = "")
		{
			...
			string userId = PulseAppSettings.GetUserID();
			...
			string token = PulseAppSettings.GetToken();
