...
{
	public class SettingsView : PulseAppView
	{
		...
		private Label BuildSectionHeader(string text)
		{
			...
			header.FontSize = 13;
			header.FontAttributes = FontAttributes.Bold;
			header.TextColor = PulseAppColors.Accent;
		...
		private Button BuildSegmentButton(string text)
		{
			...
			button.BackgroundColor = PulseAppColors.Surface;
		...
		private View BuildCachingSection()
		{
			...
			m_usageBar.ProgressColor = PulseAppColors.Accent;
			...
			m_refreshCacheButton.TextColor = PulseAppColors.OnBackground;
			m_refreshCacheButton.BackgroundColor = PulseAppColors.Surface;
			...
			m_clearCacheButton.TextColor = PulseAppColors.OnBackground;
			m_clearCacheButton.BackgroundColor = PulseAppColors.Surface;
		...
		private View BuildDiagnosticsSection()
		{
			...
			exportButton.TextColor = PulseAppColors.OnBackground;
			exportButton.BackgroundColor = PulseAppColors.Surface;
			...
			resetLogButton.TextColor = PulseAppColors.OnBackground;
			resetLogButton.BackgroundColor = PulseAppColors.Surface;
		...
		private View BuildLoginSection()
		{
			...
			connectButton.TextColor = PulseAppColors.Background;
			connectButton.BackgroundColor = PulseAppColors.Accent;
			...
			m_connectStatusLabel.TextColor = PulseAppColors.TextSecondary;
			section.Children.Add(m_connectStatusLabel);

			Button logoutButton = new Button();
			logoutButton.Text = "Log out";
			logoutButton.TextColor = PulseAppColors.OnBackground;
			logoutButton.BackgroundColor = PulseAppColors.Surface;
		...
		public override void Initialize()
		{
			...
			SetNormalize(PulseAppSettings.GetNormalizeVolume());

			int prefetch = PulseAppSettings.GetPrefetchCount();
			m_prefetchSlider.Value = prefetch;
			m_prefetchValueLabel.Text = "Prefetch Tracks: " + prefetch;

			long limitBytes = PulseAppSettings.GetCacheLimitBytes();
		...
		private void OnNormalizeOffClicked(object sender, EventArgs e)
		{
			...
			PulseAppSettings.SetNormalizeVolume(m_normalize);
		}
		...
		private void OnPrefetchChanged(object sender, ValueChangedEventArgs e)
		{
			...
			PulseAppSettings.SetPrefetchCount(count);
		}
		...
		private void OnCacheSizeChanged(object sender, ValueChangedEventArgs e)
		{
			...
			PulseAppSettings.SetCacheLimitBytes(bytes);
		...
		private void OnRefreshCacheClicked(object sender, EventArgs e)
		{
			...
			cache.ExecuteAsync(() =>
			{
				PulseAppCacheStats stats = cache.GetCacheStats();
		...
		private void OnConnectClicked(object sender, EventArgs e)
		{
			...
			bool serverChanged = ip != PulseAppSettings.GetServerIp()
				...
				|| m_useHttps != PulseAppSettings.GetUseHttps();
			if (serverChanged)
			{
		...
		private void ApplyCacheStats(PulseAppCacheStats stats)
		{
			long limitBytes = PulseAppSettings.GetCacheLimitBytes();
