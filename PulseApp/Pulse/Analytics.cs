...
namespace PulseApp.Pulse
{
	...
	public class Analytics
	{
		...
		public Analytics(MediaClient client)
		{
			...
			flushThread.Name = "PulseAppAnalyticsFlush";
			flushThread.Start();
		}
		...
		private void Record(eAction action, eResult result, ePulseWireType objectType, string objectId, float durationMs, string detail)
		{
			...
			try
			{
				...
				try
				{
					enabled = PulseAppSettings.GetAnalyticsEnabled();
				}
				catch (Exception ex)
				{
		...
		private void FlushOnce()
		{
			try
			{
				...
				if (!PulseAppSettings.GetAnalyticsEnabled())
				{
					return;
				}
				...
				batch.DeviceId = PulseAppSettings.GetOrCreateDeviceId();
				batch.SessionId = m_sessionId;
				batch.UserID = PulseAppSettings.GetUserID();
				batch.AppVersion = "";
				try
				{
