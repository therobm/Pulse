...
namespace PulseApp.Views
{
	...
	public class LibraryView : PulseAppView
	{
		...
		private Button BuildSortButton(string text)
		{
			Button button = new Button();
			button.Text = text;
			button.TextColor = PulseAppColors.TextSecondary;
		...
		private View BuildLists()
		{
			...
			m_artistsList.BackgroundColor = PulseAppColors.Background;
			listContainer.Children.Add(m_artistsList);

			m_albumsList = new CollectionView();
			m_albumsList.IsVisible = false;
			m_albumsList.BackgroundColor = PulseAppColors.Background;
		...
		private View BuildLetterOverlay()
		{
			...
			panel.BackgroundColor = PulseAppColors.Surface;
			panel.Stroke = new SolidColorBrush(PulseAppColors.Divider);
		...
		private View BuildLetterGrid()
		{
			...
			for (int index = 0; index < totalLetters; index++)
			{
				...
				letterButton.TextColor = PulseAppColors.OnBackground;
				letterButton.BackgroundColor = PulseAppColors.SurfaceElevated;
		...
		private void SetActiveSort(eLibrarySort sort)
		{
			...
			m_sortAlphabetical.TextColor = PulseAppColors.TextSecondary;
			m_sortDateReleased.TextColor = PulseAppColors.TextSecondary;

			if (sort == eLibrarySort.Alphabetical)
			{
				m_sortAlphabetical.TextColor = PulseAppColors.Accent;
			}
			else if (sort == eLibrarySort.DateReleased)
			{
				m_sortDateReleased.TextColor = PulseAppColors.Accent;
			}
