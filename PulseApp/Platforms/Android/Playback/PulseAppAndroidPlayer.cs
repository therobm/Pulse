...
{
	public class PulseAppAndroidPlayer : IMediaPlayer
	{
		...
		public PulseAppAndroidPlayer(MainView mainView, MediaClient mediaClient)
		{
			m_mainView = mainView;
			m_data = mediaClient;

			PulseAppMediaLibraryService.s_mediaClient = m_data;

			Context context = Android.App.Application.Context;
			ComponentName componentName = new ComponentName(context, Java.Lang.Class.FromType(typeof(PulseAppMediaLibraryService)));
		...
		private void CacheQueued(int queueID)
		{
			...
			{
				Log.Info("PulseAppPlayer: Cancelled existing download queue for replacement");
				return;
			}
